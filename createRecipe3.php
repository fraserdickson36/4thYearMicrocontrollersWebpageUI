...
<div class="main center">
  ...
  <div class="row text-center padding">
    ...
    <div class="padding col-xs-12 col-sm-12 col-md-12">
      ...
      <form action = "insertRecipe4.php" method = "POST">
        <h3 class = "hb3 col-8 mx-auto">Target Device:</h3>
        <select class="form-select form-select-lg mb-3 col-8 mx-auto" name= "TargetDevice" aria-label=".form-select-lg example" required>

        <?php
        $resultTargetDevice = $mysqli->query("SELECT d_Name FROM registereddevices");

        while($rows = $resultTargetDevice -> fetch_assoc())
        {
          //output all of the registered devices, except if the action peripheral includes a NaN, into the dropdown menu
          $targetdeviceName = $rows['d_Name'];
          $result = $mysqli->query("SELECT * FROM registereddevices WHERE d_Name = '$targetdeviceName'");
          ...
          if($ID != "5")
          { 
            echo "<option value = '$targetdeviceName'>$targetdeviceName</option>";
          }
        }
        ?>
        </select>
