...
//Remove unfulfilledd recipes
$Delete1 = "DELETE FROM recipes where r_TriggerDevice ='NULL' OR r_TriggerDevice IS NULL;";
$Delete2 = "DELETE FROM recipes where r_TriggerPeripheral ='NULL' OR r_TriggerPeripheral IS NULL;";
$Delete3 = "DELETE FROM recipes where r_Trigger ='NULL' OR r_Trigger IS NULL;";

$stmt = $mysqli->prepare($Delete1);            
$stmt->execute(); 
$stmt->close();
$stmt = $mysqli->prepare($Delete2);
$stmt->execute(); 
$stmt->close();
$stmt = $mysqli->prepare($Delete3);
$stmt->execute(); 
$stmt->close();

$Delete1 = "DELETE FROM recipes where r_TargetDevice ='NULL' OR r_TargetDevice IS NULL;";
$Delete2 = "DELETE FROM recipes where r_ActionPeripheral ='NULL' OR r_ActionPeripheral IS NULL;";
$Delete3 = "DELETE FROM recipes where r_Action ='NULL' OR r_Action IS NULL;";
...
<body>
    ...
    <div id="refresh"></div>
    <button type="submit" class="btn btn-primary" onclick="window.location.href='index.php';">New Recipe</button>
...
$('#refresh').load('recipesAuto.php');
$(document).ready(function(){
    setInterval(function(){
    $('#refresh').load('recipesAuto.php'); //load php script that loads table
    },5000); //delay 5000ms
});
</script> 
