...
<div class="main center">
  ...
  <div class="row text-center padding">
    ...
    <div class="padding col-xs-12 col-sm-12 col-md-12">
      ...
      <form action = "insertRecipe2.php" method = "POST">
        <h3 class = "hb3 col-8 mx-auto">If this:</h3>
        <select class="form-select form-select-lg mb-3 col-8 mx-auto" name = "TriggerPeripheral" aria-label=".form-select-lg example" required>

          <?php
          // Find all trigger peripherals available, then find respective descriptions and echo them onto dropdown menu options

          $SearchQuery = $mysqli->query("SELECT r_TriggerDevice FROM recipes WHERE r_id=(SELECT max(r_id) FROM recipes)");          
          $Search = $SearchQuery->fetch_assoc();
          $SelectedDevice = $Search['r_TriggerDevice'];
          ...
          $tempQuery = $mysqli->query("SELECT tp_id FROM catalogueperipherals WHERE cat_DeviceName = '$SelectedDevice'");

          while($rows = $tempQuery -> fetch_assoc())
          {
            $temprows = $rows['tp_id'];
            $resultTriggerPeripheral = $mysqli->query("SELECT tp_Name FROM triggerperipherals WHERE tp_id = '$temprows'");
            while($result = $resultTriggerPeripheral -> fetch_assoc())
            
              $triggerPeripheralSelect = $result['tp_Name']; 
              $resultTriggerDesc = $mysqli->query("SELECT * FROM triggerperipherals WHERE tp_Name = '$triggerPeripheralSelect'");
              $descResult = $resultTriggerDesc->fetch_assoc();
              $triggerSelect = $descResult['tp_Description'];   
              echo "<option value = '$triggerPeripheralSelect'>$triggerSelect</option>";
          }
          ?>
        </select>
        <div class="col-12">
