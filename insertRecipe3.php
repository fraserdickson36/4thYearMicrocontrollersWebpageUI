...
$r_Trigger = $_POST['Trigger'];

// Insert trigger device into recipes
$INSERT = "UPDATE recipes SET r_Trigger = ? WHERE r_Name = '$r_Name'";


            $stmt = $conn->prepare($INSERT);
            $stmt->bind_param("s", $r_Trigger);
            $stmt->execute();
...
if ($done)
{
    header("Location: createRecipe3.php");
    exit();
}
?>