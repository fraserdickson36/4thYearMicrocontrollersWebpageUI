...
<div class="main center">
  ...
  <div class="row text-center padding">
    ...
    <form action = "insertRecipe5.php" method = "POST">
        <h3 class = "hb3 col-8 mx-auto">Then this:</h3>
        <select class="form-select form-select-lg mb-3 col-8 mx-auto" name = "ActionPeripheral" aria-label=".form-select-lg example" required>
          <?php
          //output all of the action peripherals associated with the action device chosen into the dropdown menu
          $previous = '';
          $SearchQuery = $mysqli->query("SELECT r_TargetDevice FROM recipes WHERE r_id=(SELECT max(r_id) FROM recipes)");          
          $Search = $SearchQuery->fetch_assoc();
          $SelectedDevice = $Search['r_TargetDevice'];
          $SearchQuery = $mysqli->query("SELECT d_DeviceType FROM registereddevices WHERE d_Name = '$SelectedDevice'");          
          $Search = $SearchQuery->fetch_assoc();
          $SelectedDevice = $Search['d_DeviceType'];
          $tempQuery = $mysqli->query("SELECT ap_id FROM catalogueperipherals WHERE cat_DeviceName = '$SelectedDevice'");

          while($rows = $tempQuery -> fetch_assoc())
          {
            $temprows = $rows['ap_id'];
            $resultActionPeripheral = $mysqli->query("SELECT ac_Name FROM actionperipherals WHERE ap_id = '$temprows'");
            while($result = $resultActionPeripheral -> fetch_assoc())
              $actionPeripheralSelect = $result['ac_Name'];
              $actionDesc = $mysqli->query("SELECT * FROM actionperipherals WHERE ac_Name = '$actionPeripheralSelect'");
              $descResult = $actionDesc->fetch_assoc();
              $actionPeripheralDesc = $descResult['ap_Description'];
              if($actionPeripheralSelect != $previous)
              {
                echo "<option value = '$actionPeripheralSelect'>$actionPeripheralDesc</option>";
              }                 
            
              $previous = $actionPeripheralSelect;
          }
          ?>
        </select>
        <div class="col-12">
