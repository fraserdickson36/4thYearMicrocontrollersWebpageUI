...
<div class="main center">
  ...
  <div class="row text-center padding">
    ...
    <form action = "insertRecipe6.php" method = "POST">
        <h3 class = "hb3 col-8 mx-auto">Does this:</h3>
        <select class="form-select form-select-lg mb-3 col-8 mx-auto" name = "Action" aria-label=".form-select-lg example" required>
          <?php

            //output all of the actions associated with the chosen action peripheral into the dropdown menu options
              $SearchQuery = $mysqli->query("SELECT r_ActionPeripheral FROM recipes WHERE r_id=(SELECT max(r_id) FROM recipes)");          
              $Search = $SearchQuery->fetch_assoc();
              $SelectedPeripheral = $Search['r_ActionPeripheral'];
              $idQuery = $mysqli->query("SELECT ap_id FROM actionperipherals WHERE ac_Name = '$SelectedPeripheral'");
              $idSearch = $idQuery->fetch_assoc();
              $SelectedID = $idSearch['ap_id'];
              
              $tempQuery = $mysqli->query("SELECT ac_id FROM actionconfigurations WHERE ap_id = '$SelectedID'");
              while($rows = $tempQuery -> fetch_assoc())
              {
                $temprows = $rows['ac_id'];
                $resultActionConfig = $mysqli->query("SELECT * FROM actions WHERE ac_id = '$temprows'");
                while($result = $resultActionConfig -> fetch_assoc())
                  
                  $actionName = $result['ac_Name'];
                  $resultActionDesc = $mysqli->query("SELECT * FROM actions WHERE ac_Name = '$actionName'");
                  $descResult = $resultActionDesc->fetch_assoc();
                  $actionSelect = $descResult['ac_Description'];
                  echo "<option value = '$actionName'>$actionSelect</option>";
              }
          ?>
        </select>
        <div class="col-12">
          <button class="btn btn-secondary mb-3" type="submit">Done</button>
        </div>
